...
from libecalc.domain.process.value_objects.fluid_stream import FluidStream, FluidService
...
class CommonStreamDistribution(StreamDistribution, Generic[T]):
    def __init__(
        ...
        assert len(items) == len(overflows)
    ...
    def _get_sorted_items(self) -> Iterable[T]:
        ...
        for overflow in self._overflows:
            graph.add_edge(overflow.from_id, overflow.to_id)
